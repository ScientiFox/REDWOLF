####
#   ZIM Archive interactions for REDWOLF
####
...
class MyHTMLParser(HTMLParser):
    ...
    def get_parsed(self,text):
        ...
        #print([s[0].split(">")[1].split("<")[0] for s in sections])
...
class archive:
    ...
    def suggestions(self,target,num=20):
        ...
        sugg_list = list(self.recommended.getResults(0,num))
        sugg_list = [(s,SequenceMatcher(None,s,target).ratio()) for s in sugg_list]
        sugg_list.sort(key=lambda x:x[1],reverse=True)
        sugg_list = [s[0] for s in sugg_list]
        return sugg_list
    ...
    def grab(self,link):
        entry = self.pull_article(link)
        art_out = self.parser.get_parsed(entry)
        return art_out
